...
import { 
  ...
  Send, Sparkles, Lightbulb, MessageSquare, RefreshCcw, Lock
} from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { debounce } from 'lodash'; // Added for debouncing localStorage writes
 ...
 * - AI-powered personalized support via Claude API
 */
const SobrietySupport = () => {
  ...
  // Save data when important state changes using debounce to reduce writes
  ...
  }, [streak, achievements, userName, sobrietyDate, journalEntries, triggers, debouncedSave]);
  
  // Cooldown timer for API requests
  useEffect(() => {
  ...
  // Save API key and enable features - now using sessionStorage for better security
  const saveApiKey = () => {
    if (apiKey) {
      ...
      generatePersonalizedMessage();
      generateJournalPrompt();
      setMessage("AI personalization enabled for this session!");
    }
  };
  ...
  const generatePersonalizedMessage = async () => {
    ...
    if (now - lastMessageTimestamp < COOLDOWN_PERIOD) {
      ...
      setMessage(`Please wait ${remainingSeconds} seconds before generating another message.`);
      return;
    }
  ...
  const handleUrgeSupportSubmit = async (e) => {
    ...
    setUrgeSupportMessages(prevMessages => [...prevMessages, userMessage]);
    ...
    try {
      ...
      if (response) {
        ...
        setUrgeSupportMessages(prevMessages => [...prevMessages, assistantMessage]);
      ...
      setUrgeSupportMessages(prevMessages => [...prevMessages, fallbackMessage]);
    } finally {
        ...
        {showSettings && (
          ...
          >
            <Card className="border-blue-200 bg-blue-50 mb-4">
              ...
              <Button onClick={addJournalEntry} className="w-full mt-2" aria-label="Save journal entry">
            ...
            <CardContent className="p-4 space-y-4">
              ...
              <div className="space-y-3 pt-3 border-t">
                ...
                {triggers.map((trigger) => (
                  <Card 
                    key={trigger.id} 
                    className={`border ${getTriggerColor(trigger.intensity)}`}
                  >
                    <CardContent className="p-3">
                      <div className="flex items-center justify-between">
                        ...
                        <Badge 
                          variant="outline" 
                          className={`text-xs ${getTriggerColor(trigger.intensity)}`}
                        >
...
export default SobrietySupport;</CardHeader>
                        ...
                        <TooltipProvider>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button variant="ghost" size="icon" className="mt-2 ml-2 h-9 w-9">
                                <Info className="h-4 w-4" />
                              </Button>
                            </TooltipTrigger>
                            <TooltipContent className="max-w-xs">
                              <p>Your API key will be stored in session storage and will be cleared when you close the browser.</p>
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                      </div>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-4 pt-4 border-t">
                  ...
                  {triggers.map((trigger) => (
                    <Card 
                      key={trigger.id} 
                      className={`bg-gray-50 border ${getTriggerColor(trigger.intensity)}`}
                    >
                      <CardContent className="p-4">
                        <div className="flex items-center justify-between">
                          <div>
                          ...
                          <div className="text-right">
                            <Badge 
                              variant="outline" 
                              className={getTriggerColor(trigger.intensity)}
                            >
      ...
      <div className="md:hidden">
        ...
        {activeTab === "journal" && (
          <Card>
            <CardContent className="p-4 space-y-4">
              {/* Mobile AI Journal Prompts */}
              {isApiConfigured && personalizationEnabled && (
                <Button 
                  variant="outline" 
                  size="sm" 
                  onClick={() => setShowJournalPrompts(!showJournalPrompts)}
                  className="text-purple-600 flex items-center gap-2 w-full justify-center mb-2"
                >
                  <Lightbulb className="w-4 h-4" />
                  {showJournalPrompts ? "Hide Prompts" : "Get Writing Prompts"}
                </Button>
              )}
              ...
              <AnimatePresence>
                {showJournalPrompts && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, height: 'auto' }}
                    exit={{ opacity: 0, height: 0 }}
                    className="bg-purple-50 p-3 rounded-md mb-3"
                  >
                    <div className="flex flex-col">
                      <div className="flex items-start gap-2 mb-2">
                        ...
                        <div>
                          {isGeneratingPrompt ? (
                            <div className="animate-pulse text-purple-600 text-xs">
                              Generating prompt...
                            </div>
                          ) : (
                            <p className="text-xs text-purple-700">{currentJournalPrompt || "No prompt available yet."}</p>
                          )}
                        </div>
                      </div>
